...
// Èítaè pøijatých impulzù
uint8_t counter = 0;
// Promìnná pro doèasné uložení mezivýsledku
uint16_t time = 0;

// Vyhrazené místo v pamìti pro pøijatá data
uint8_t data[RADIO_MSGMAXLEN];
// Promìnná pro práci s polem
uint8_t data_index = 0;
// Promìnná pro práci s polem bitù
uint8_t bitIndex = 0;
// Èítaè pøijatých bitù
 ...
 *
 * -----------------------------------------------------------------------------
 * Resetuje èasovaè a pokraèuje v èítání.
 *******************************************************************************/
void RADIO_resetAndStartCount(void)
{
	TCNT1 = 0x0000;
 ...
 *
 * -----------------------------------------------------------------------------
 * Uvede dekódování radiopøijímaèe do výchozího stavu.
 *******************************************************************************/
void RADIO_InitAndStart(void)
{
	progress = 0x00;
 ...
 *
 * -----------------------------------------------------------------------------
 * Procedura zajišující úkony potøebné pro dokonèení pøíjmu dat.
 *******************************************************************************/
void RADIO_sequenceEnd(void)
{
 ...
 *
 * -----------------------------------------------------------------------------
 * Procedura zajišující uložení bitu do pole pøijatých hodnot.
 *******************************************************************************/
void RADIO_bitReceived(uint8_t state)
{
	++bitCounter;
 ...
 *
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 * TUTO PROCEDURU VOLEJTE POKAŽDÉ DOJDE-LI KE ZMÌNÌ NA VSTUPU OD RADIOPØIJÍMAÈE!
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 *
 *******************************************************************************/
void RADIO_ISR(void)
{
	switch(progress)
	{
		...
		case 0x01:
			...
			return;
		case 0x02:
			if(bitRead(RADIO_PIN, RADIO_DAT) == 0)//byla tam jednièka?
			{
				time = RADIO_timerRead();
